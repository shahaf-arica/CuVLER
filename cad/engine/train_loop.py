...
# Code from https://github.com/facebookresearch/CutLER/blob/main/cutler/engine/train_loop.py
    ...
    def copy_and_paste(self, labeled_data, unlabeled_data):
        ...
        for cur_labeled_data, cur_unlabeled_data in zip(labeled_data, unlabeled_data):
            ...
            if num_labeled_instances == 0 or num_copy == 0:
                ...
                cur_labeled_image_new = F.interpolate(cur_labeled_image[None, ...].float(), size=(h_new, w_new),
                                                      mode="bilinear", align_corners=False).byte().squeeze(0)
                if isinstance(copied_masks, torch.Tensor):
                    masks_new = F.interpolate(copied_masks[None, ...].float(), size=(h_new, w_new), mode="bilinear",
                                              align_corners=False).bool().squeeze(0)
                else:
                    masks_new = F.interpolate(copied_masks.tensor[None, ...].float(), size=(h_new, w_new),
                                              mode="bilinear", align_corners=False).bool().squeeze(0)
                copied_boxes.scale(1. * unlabeled_w / labeled_w * resize_ratio,
                                   1. * unlabeled_h / labeled_h * resize_ratio)
