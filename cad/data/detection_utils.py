...
# Original code from https://github.com/facebookresearch/detectron2/blob/main/detectron2/data/detection_utils.py
# Code Modified by Shahaf Arica from https://github.com/facebookresearch/CutLER/blob/main/cutler/data/detection_utils.py
...
from cad.data import transforms as T
...
def annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    target.gt_classes = torch.zeros(len(annos), dtype=torch.int64)

    scores = [float(obj["score"]) for obj in annos if "score" in obj]
    if len(scores) == 0:
        target.gt_scores = torch.ones(len(annos), dtype=torch.float32)
    else:
        target.gt_scores = torch.tensor(scores, dtype=torch.float32)
