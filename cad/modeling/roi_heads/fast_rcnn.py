...
# Original code from https://github.com/facebookresearch/detectron2/blob/main/detectron2/modeling/roi_heads/fast_rcnn.py
# Modified by Shahaf Arica from https://github.com/facebookresearch/CutLER/blob/main/cutler/modeling/roi_heads/fast_rcnn.py
...
from cad.modeling.box_regression import _soft_dense_box_regression_loss
...
def fast_rcnn_inference(
    ...
    Args:
        boxes (list[Tensor]): A list of Tensors of predicted class-specific or class-agnostic
            ...
            This is compatible with the output of :meth:`FsdetFastRCNNOutputLayers.predict_boxes`.
        scores (list[Tensor]): A list of Tensors of predicted class scores for each image.
            Element i has shape (Ri, K + 1), where Ri is the number of predicted objects
            for image i. Compatible with the output of :meth:`FsdetFastRCNNOutputLayers.predict_probs`.
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def from_config(cls, cfg, input_shape):
        return {
            ...
            "use_soft_targets"          : cfg.MODEL.ROI_HEADS.USE_SOFT_TARGETS,
            # fmt: on
        }
    ...
    def losses(self, predictions, proposals, weights=None):
        ...
        gt_score = cat([p.gt_scores for p in proposals], dim=0) if self.use_soft_targets else None

        if self.use_sigmoid_ce:
            loss_cls = self.sigmoid_cross_entropy_loss(scores, gt_classes)
        else:
            if self.use_soft_targets:
                fg_prob = gt_score.detach().clone()
                # for each element that is in background class its foreground probability should be 0
                fg_prob[gt_classes == self.num_classes] = 0.0
                bg_prob = 1 - fg_prob
                # for object detection the first class is foreground and the last class is background
                gt_targets = torch.stack((fg_prob, bg_prob), dim=1)
            else:
                gt_targets = gt_classes
            if weights is not None:
                loss_cls = (weights * cross_entropy(scores, gt_targets, reduction='none')).mean()
            else:
                loss_cls = cross_entropy(scores, gt_targets, reduction="mean")

        losses = {
            "loss_cls": loss_cls,
            "loss_box_reg": self.box_reg_loss(
                proposal_boxes, gt_boxes, proposal_deltas, gt_classes, gt_score
            ),
        }
    ...
    def box_reg_loss(self, proposal_boxes, gt_boxes, pred_deltas, gt_classes, gt_score=None):
        """
        Args:
        ...
        if gt_score is None:
            ...
            gt_score = gt_score.detach()
            loss_box_reg = _soft_dense_box_regression_loss(
                [proposal_boxes[fg_inds]],
                self.box2box_transform,
                [fg_pred_deltas.unsqueeze(0)],
                [gt_boxes[fg_inds]],
